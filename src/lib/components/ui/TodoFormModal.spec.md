...
### 마감 시각
- [x] **입력 필드**: `#due-time` (`input[type=time]`) 렌더링, 값이 있으면 "지우기" 버튼 노출
- [x] **생성 제출**: 입력한 시각이 `due_time: 'HH:mm'`으로 전달됨
- [x] **미입력 제출**: `due_time: null`로 전달됨 (날짜 단위 관리, 하위 호환)
- [x] **수정 모드 값 채움**: Supabase `time`의 `'HH:MM:SS'`가 `'HH:mm'`으로 채워짐
- [x] **수정 제출**: `updates.due_time`이 포함됨

### Layout
- [x] **기본 최소 높이**: 패널에 `min-h-[467px]` 적용 (실측 기본 높이 427px + 40px)
- [x] **여백 흡수**: 늘어난 높이를 상세 내용 `textarea`가 `flex-1`로 흡수 (80px → 120px)
